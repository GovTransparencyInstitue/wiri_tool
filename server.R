...
function(input, output, session) {
  ...
  observeEvent(input$file1, {
    ...
    if (tolower(tools::file_ext(input$file1$name)) == "csv") {
      output$file_message1 <- renderText("File is a CSV")
  ...
  observeEvent(input$file2, {
    ...
    if (tolower(tools::file_ext(input$file2$name)) == "csv") {
      output$file_message2 <- renderText("File is a CSV")
    } else {
  ...
  # General Parameters -----------------------------------------------------------
  n_areas = 10
  df_contracts = input_contracts() # read.csv("data/peru_contracts_example.csv")
  df_survey =  input_survey() # input$df_survey
...
# Create a histogram plot and render it in the UI
output$histogram_plot <- renderPlot({
  
  data_hist <- df_wiri_cross() 

  ggplot(data_hist, aes(x = WIRI)) +
...
output$wiri_csv <- downloadHandler(
  filename = function() {
    "wiri.csv"  # Name of the downloaded CSV file
  },
